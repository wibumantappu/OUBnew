...
# ========================= CONSTANTS ============================
UNAPPROVED_MSG = (
    "`Euy euy! bot x√sun3 disini.\n\n`"
    "`Anda belum diterima untuk chat dengan pemilik akun ini.`"
    "`Silahkan bercermin untuk melihat kadar wajah anda. Apakah sudah lulus uji untuk chat dengan pemilik akun ini.\n\n`"
    "`Kalo gak sabar nunggu, mending isi pulsa 50k ke nomor 0895358957539!`"
)
# =================================================================
    ...
    if (
        ...
        if not apprv and event.text != UNAPPROVED_MSG:
            ...
            if COUNT_PM[event.chat_id] > 10:
                await event.respond(
                    "`Dibilangin gak usah spam ngeyel amat. Bisa sabar kagak?.`\n"
                    "`Selamat, anda telah terblokir :) Silahkan isi pulsa 50K ke 0895358957539 agar ter-unblock.`"
                )
...
async def approvepm(apprvpm):
    ...
    try:
        ...
        await apprvpm.edit("`Kan udah bisa chat. Pikun ya?.`")
        return

    await apprvpm.edit(f"[{name0}](tg://user?id={uid}) `Sep bro, dah ane approve!`")
...
async def disapprovepm(disapprvpm):
    ...
    await disapprvpm.edit(
        f"[{name0}](tg://user?id={disapprvpm.chat_id}) `Ane gak terima PM dari lu bro!`"
    )
    ...
    if BOTLOG:
        await disapprvpm.client.send_message(
            ...
            " Selamat, anda tertolak untuk chattingan dengan saya.",
        )
...
async def blockpm(block):
    ...
    if block.reply_to_msg_id:
        ...
        await block.edit("`Tjie dah ane block!`")
        uid = replied_user.id
    else:
        await block.client(BlockRequest(block.chat_id))
        aname = await block.client.get_entity(block.chat_id)
        await block.edit("`Waduh dah ada yang diblock nih!`")
        name0 = str(aname.first_name)
        uid = block.chat_id
